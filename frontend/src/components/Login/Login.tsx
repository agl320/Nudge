          ...
          <div className="lg:flex h-full justify-center">
            <div className="max-w-1/2 w-full my-8 bg-white/10 lg:mr-4 rounded-md flex flex-col justify-center">
              <section className="space-y-8 p-16 text-center">
                ...
                <p className="mt-4 max-w-xs mx-auto">Prioritize meeting efficiency by optimizing participant time.</p>
                <div className="text-sm bg-fade inline-block p-4 rounded-md">
                  <p className="mt-4 text-white/50">
                    <span className="text-green-400/50">{">"}</span> Loading modules...
                  </p>
                  <p className="mt-4 text-white/70">
                    <span className="text-green-400/70">{">"}</span> Optimizing meetings...
                  </p>
                  <p className="mt-4">
                    <span className="text-green-400">{">"}</span> Cleaning up convos...
                  </p>
                  <p className="mt-4 text-green-400 font-bold">[ *_* ]</p>
                </div>
              </section>
            </div>
            <div className="max-w-1/2 w-full my-8 lg:ml-4 text-white rounded-md flex flex-col justify-center p-8 py-16">
              <div className="max-w-sm mx-auto">
                {isOnRegister && <RegisterForm setIsOnRegister={setIsOnRegister} />}
                {!isOnRegister && <LoginForm setIsOnRegister={setIsOnRegister} />}
              </div>
            </div>
          </div>
        </section>
