...
export default function Meeting() {
  ...
  const setupWebRTC = async () => {
    ...
    socket.on("signal", async ({ sender, sdp, candidate }: SignalPayload) => {
    ...
    socket.on("outlier_detected", ({ user_id, sentence }: { user_id: string; sentence: string }) => {});
  ...
  const joinMeeting = async () => {
    ...
    socket.on("transcription", ({ timestamp, user_id, sentence }) => {
      setChatStream((prev) => [...prev, `[${timestamp}] User ${user_id} said: ${sentence}`]);
    });
  };
  ...
  const displayedStreams = Object.entries(remoteStreams).slice(currentSliceIndex, currentSliceIndex + 3);

  const [onTopic, setOnTopic] = useState<string>(exampleTimeBlockData[1].label);
      ...
      <div className="h-full w-full bg-black max-w-full mx-auto flex flex-col flex-1 px-24">
        ...
        <div className="flex py-8 h-full">
          <div className="bg-white/10 rounded-md p-2 mr-auto w-[700px] p-4 text-sm h-full flex flex-col justify-between">
            <div>
              <p className="text-white text-lg mb-4 font-semibold">Settings</p>
              ...
              <InfoBar className="mt-4" label={"active-participants"} value={Object.keys(remoteStreams).length + 1} />
              <Separator className="w-full bg-white/15 my-4" />
              <p className="text-md font-medium mb-4">User info</p>
              <InfoBar label={" user-uid"} value={user?.uid ?? ""} />
              <InfoBar className="mt-4" label={"user-displayName"} value={user?.displayName ?? ""} />
              ...
              <div className="w-full h-[150px] mt-4">
                {exampleTimeBlockData.map((timeBlock, index) => {
                  // Generate a color based on the index, scaling from orange to yellow
                  const colorScaler = (index, total) => {
                    const green = Math.round(165 + (90 * index) / (total - 1)); // Green increases from 165 to 255
                    return `rgba(${green}, 255, 20, ${onTopic === timeBlock.label ? "1" : "0.50"})`; // RGB value for orange-to-yellow transition
                  };
                  ...
                  return (
                    <div
                      ...
                      style={{
                        height: `${timeBlock.size * 100}%`,
                        backgroundColor: colorScaler(index, exampleTimeBlockData.length), // Set background color
                      }}
                      className="rounded-md flex items-center justify-center text-xs text-black font-semibold mt-1"
                    >
                      {timeBlock.label} {/* Display the label for each block */}
                    </div>
                  );
                })}
              </div>
              ...
              <div className="">
                <div className="flex justify-between">
                  <p className="text-md font-medium">Voice Sensitivity</p>
                  <p className="bg-green-500/25 text-green-500 px-2 py-1 rounded-md text-xs">{sliderValue}%</p>
                </div>
            ...
            <div>
              ...
              <div className="flex mt-8">
                <Button className="text-black mr-2 bg-white w-1/2" onClick={toggleMute}>
                  {isAudioMuted ? "Unmute" : "Mute"}
                </Button>
                <Button className="text-white ml-2 bg-red-500 w-1/2" onClick={leaveMeeting}>
                  Leave Meeting
                </Button>
              </div>
            </div>
          ...
          <div className="w-full px-8">
              ...
              {displayedStreams.map(([id, stream]) => (
                <video
                  ...
                  style={{
                    ...
                    width: "33.3333%",
                    height: "auto",
                    border: `4px solid ${talkingRemoteUsers.has(id) ? "#3ba55d" : "transparent"}`,
            ...
            <div className="flex justify-between mt-4">
              <Button className="bg-white/15" onClick={handleLeftClick}>
                <ArrowLeft />
              </Button>
              <Button className="bg-white/15" onClick={handleRightClick}>
                <ArrowRight />
              </Button>
            </div>
          </div>
          <div className="bg-white/10 rounded-md p-4 mr-auto w-[700px] h-full flex flex-col">
            <div>
              <p className="text-white text-lg mb-4 font-semibold">Nudge</p>
              <Separator className="w-full bg-white/15 my-4" />
              ...
              <Button className="text-white bg-green-600 w-full mt-4" onClick={leaveMeeting}>
                Pause Topic-Check
              </Button>
            </div>
            <div className="w-full flex flex-col flex-grow rounded-md mt-4 justify-center">
              <div className="flex justify-center">
                <div className="text-center font-display text-2xl">
                  <p className="text-green-500 font-bold">ヽ(・∀・)ﾉ</p>
